	...
	<div class="modal-dialog" style="width: 800px; height: 1000px">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				</input> <input type="button" value="通过" onclick="apply(1)"
					class="btn btn-primary"> <input type="button" value="拒绝"
					onclick="apply(2)" class="btn btn-danger"> <input
					type="button" value="热门" onclick="setHot(1)"
					class="btn btn-warning" /> <input type="button" value="非热门"
					onclick="setHot(0)" class="btn btn-info" />
	...
	<tbody>
		<c:forEach items="${pageInfo.list}" var="article">
			<tr class="active">
				...
				<td><input type="button" onclick="toApply(${article.id})"
					value="审核" class="btn-info" /> <input type="button"
					onclick="delArticle(${article.id})" value="删除" class="btn-danger" />
				</td>
			</tr>
...
<ul class="pagination">
	...
	<c:forEach begin="${pageInfo.pageNum-2 > 1 ? info.pageNum-2:1}"
		end="${pageInfo.pageNum+2 > info.pages ? info.pages:info.pageNum+2}"
		varStatus="index">
		<c:if test="${pageInfo.pageNum!=index.index}">
			<li><a href="javascript:goPage(${index.index})">${index.index}</a></li>
		</c:if>
		<c:if test="${pageInfo.pageNum==index.index}">
			<li><a href="javascript:void"><strong>
