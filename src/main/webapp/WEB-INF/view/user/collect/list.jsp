		...
		<tbody>
			<c:forEach items="${info.list}" var="collect">
				<tr>
					...
					<td>${collect.name}</td>
					<td>${collect.url}</td>
					<td><fmt:formatDate pattern="YYYY年MM月dd号 HH:mm:ss"
							value="${collect.created}"></fmt:formatDate></td>
					<td><input type="button" value="修改"
						onclick="update(${collect.id})"> &nbsp; <input
						type="button" value="删除" onclick="del(${collect.id})"></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
